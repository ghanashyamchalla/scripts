...
my $in = Bio::SearchIO->new(-format => 'blastxml', -file => $infile, -tempfile =>1);
#my $result = $in->next_result;
while (my $result = $in->next_result) {

	my $qid = $result->query_description;
	...
	#while ($hit = $result->next_hit){ #use this to report all Hits from Blast Ouput
		...
		my $align_length_query = $tiling->length('query', 'exact', $context); # length of aquery sequence in tiled alignment
		...
		my $query_identity_unrounded = $tiling->frac_identical('query', 'aligned', 'exact', $context)*100; #percent ID of the aligned part of the query (identical residues/alignment length)
		my $query_identity = sprintf "%.2f",$query_identity_unrounded;
		my $residue_cov_q_unrounded = $tiling->percent_identity('query', 'total', 'exact', $context); #percent of identitical base pairs compared to query seq length
		my $residue_cov_q = sprintf "%.2f",$residue_cov_q_unrounded;
		my $q_identities = $tiling->identities('query', 'exact', $context); #number of identical residues of query sequence in tiled alignment
